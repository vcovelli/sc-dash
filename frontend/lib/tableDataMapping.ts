...
import { Supplier, Warehouse, Product, Customer, Order } from './tableAPI';
...
// Type guard to detect Option
function isOption(obj: unknown): obj is Option {
  return typeof obj === 'object' && obj !== null;
}

// Map backend data to frontend Row format
export function mapApiDataToRows<T extends { id?: unknown }>(data: T[]): Row[] {
  return data.map((record) => ({
    ...record,
    __rowId: typeof record.id === "number" ? record.id : Number(record.id) || Math.floor(Math.random() * 1e8),
  })) as Row[];
}
...
export function getDisplayValueForReference(
  ...
  if (column.type === 'reference' && referenceData) {
    ...
    if (referencedItem) {
      ...
      // referencedItem is Option, and Option supports index signature
      return String(referencedItem[displayField] ?? referencedItem.id ?? '');
    }
  }
  return String(value ?? '');
}
...
export function validateFieldValue(
  ...
  switch (column.type) {
    ...
    case 'date':
      ...
      break;
    case 'choice':
      if (value && column.choices) {
        const isValid = Array.prototype.some.call(
          column.choices,
          (choice: Option | string) =>
            isOption(choice)
              ? (choice.value === value || choice.id === value)
              : choice === value
        );
        if (!isValid) {
...
export function transformFrontendValue(value: unknown, column: CustomColumnDef<Row>): unknown {
  ...
  switch (column.type) {
    ...
    case 'reference':
      ...
      return isOption(value) && value.id !== undefined ? value.id : value;
    default:
      return value;
  }
}
...
export function buildReferenceChoices(
  ...
  return referenceData.map(item => ({
    value: item.id,
    label: String(item[displayField] || item.name || item.id || 'Unknown'),
  }));
}
