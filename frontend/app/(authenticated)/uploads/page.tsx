...
const useUserSchema = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://192.0.2.7:8000/api/user-schema/", {
...
export default function UploadsPage() {
  ...
  useEffect(() => {
    const fetchSchema = async () => {
      const token = localStorage.getItem("access_token");
      try {
        const response = await axios.get("http://192.0.2.7:8000/api/user-schema/", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("access_token");
      await axios.post("http://192.0.2.7:8000/api/ingest-csv/", formData, {
        headers: {
