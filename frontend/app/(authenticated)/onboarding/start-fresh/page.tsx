...
export default function StartFreshPage() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const res = await axios.get("http://192.0.2.7:8000/auth/me/", {
        ...
        if (err.response?.status === 401 && refreshToken) {
          try {
            const refreshRes = await axios.post("http://192.0.2.7:8000/auth/token/refresh/", {
            ...
            const profileRes = await axios.get("http://192.0.2.7:8000/auth/me/", {
  ...
  try {
    ...
    await axios.patch("http://192.0.2.7:8000/auth/me/", {
    ...
    await axios.post("http://192.0.2.7:8000/api/user-schema/", {
