...
const ReferenceList: React.FC<ReferenceListProps> = React.memo(
  ({
    ...
    const selectedLabel = useMemo(() => {
      return options.find((opt) => String(opt.id) === String(value))?.name ?? "—";
    }, [options, value]);
    ...
    return (
      ...
      >
        <Select.Trigger
          className="inline-flex items-center justify-between w-full px-2 py-1 border border-indigo-300 rounded bg-white text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
          <ReferenceTag value={selectedLabel} />
