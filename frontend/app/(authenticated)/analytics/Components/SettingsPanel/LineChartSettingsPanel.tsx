...
import { LineChartSettings, TableMeta } from "../../types";
...
type Props = {
  ...
  onChange: (next: LineChartSettings) => void;
  mockTables: TableMeta[]
  defaultColors?: string[];
  xColumns: { name: string; type: string }[];
  yColumns: { name: string; type: string }[];
  updateSetting: (key: keyof LineChartSettings, value: unknown) => void;
...
export default function LineChartSettingsPanel({
  ...
  // settings is now LineChartSettings
        ...
        <div className="flex flex-wrap gap-3">
          {(settings.yFields ?? []).map((field, idx) => (
            <div key={field} className="flex items-center gap-2">
              ...
              <input
                type="color"
                value={settings.lineColors?.[idx] || defaultColors[idx % defaultColors.length]}
                onChange={e => {
                  const colorArr = [...(settings.lineColors ?? defaultColors)];
                  colorArr[idx] = e.target.value;
                  updateSetting("lineColors", colorArr);
                }}
